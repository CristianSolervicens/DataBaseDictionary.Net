...
namespace ModelLibrary
{
    ...
    public class MySql
    {
        ...
        static public string SQLServerConnectionString(string ServerName, string DatabaseName, string Username, string Clave, bool Async)
        {
                ...
                sConnStr += $";Application Name=DataBaseDictionary{post}";
                 
                if (Async)
                    sConnStr += ";Asynchronous Processing=True";

                sConnStr += ";Persist Security Info=False;TrustServerCertificate=True;Encrypt=True";
        ...
        public bool ExecStoredCmdData(String sCommandFile, Dictionary<string, string> Params)
        {
            ...
            if (Params.Count != 0)
            {
                List<string> keys = new(Params.Keys);
        ...
        public void EncryptStringtoFile(string strInputString, string strOutputFileName)
        {
            ...
            using FileStream outputStream = new(strOutputFileName, FileMode.OpenOrCreate, FileAccess.ReadWrite);
            ...
            CryptoStream crStream = new(outputStream, cryptic.CreateEncryptor(), CryptoStreamMode.Write);
        ...
        {
            string strFileData = "";
            using (FileStream inputStream = new(strInputFileName, FileMode.Open, FileAccess.Read))
            {
                DESCryptoServiceProvider cryptic = new();
                ...
                CryptoStream crStream = new(inputStream, cryptic.CreateDecryptor(), CryptoStreamMode.Read);

                StreamReader reader = new(crStream);
        ...
        public string GetObjectDeffinition(string ObjectName)
        {
            string Comando = $"SELECT OBJECT_DEFINITION(OBJECT_ID(N'{ObjectName}'))";
            ExecuteSqlData(Comando);
            string Content = "";
            if (Data != null)
            {
        ...
        public string BuscaPagina(string DataBaseId, string FileId, string PageId)
        {
            ...
            if (ix != -1)
            {
                ...
                ix = sAux.Contains(' ', StringComparison.CurrentCulture) ? sAux.IndexOf(' ') : sAux.IndexOf('\n');
        ...
        public bool WriteBlob(string File, string Tabla, string FieldName, string WhereConditions)
        {
            ...
            try
            {
                ...
                SqlCommand command = new(ComandoSQL);
